...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate an Enrile meme using a text.",
  cooldown: 5,
  hasPrefix: true,
  usage: "enrile text",
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (args.length === 0) {
    return api.sendMessage(applyFontMapping("❌ | Please provide a text for the Enrile meme."), tid, (err, info) => {
      setTimeout(() => api.unsendMessage(info.messageID), 3000);
    }, mid);
  }

  const text = args.join(" ").trim();
  const outputPath = __dirname + `/cache/enrile_${tid}_${mid}.png`;
