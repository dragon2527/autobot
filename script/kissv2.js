...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Kiss someone using their profile picture (v2).",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/kissv2_${userID}_${targetID}.png`;
  ...
  try {
    ...
    const kissURL = `https://api-canvass.vercel.app/kiss2?one=${userID}&two=${targetID}`;
    ...
    writer.on("finish", () => {
      api.sendMessage({
        body: formatFont(`${name1} gave a sweet kiss to ${name2}! 💋`),
