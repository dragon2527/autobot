...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "SpongeBob burning a paper with profile picture.",
  cooldown: 5,
  hasPrefix: true,
  usage: "sponge | sponge reply | sponge @mention | sponge <uid>",
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  api.getUserInfo(targetID, (err, result) => {
    ...
    const outputPath = __dirname + `/cache/sponge_${tid}_${mid}.png`;

    axios({
        method: 'get',
        url: `https://api-canvass.vercel.app/sponge?userid=${targetID}`,
        responseType: 'stream',
      })
      .then(response => {
        ...
        writer.on("finish", () => {
          api.sendMessage({
            body: formatFont(`No thanks, ${userName}, not interested in this pic! 🧽🔥 Let’s see how it burns instead!📄`),
