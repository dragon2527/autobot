...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Apply deepfry filter to profile picture.",
  cooldown: 5,
  hasPrefix: true,
  usage: "deepfry | deepfry reply | deepfry @mention | deepfry <uid>",
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  api.getUserInfo(targetID, (err, result) => {
    ...
    const outputPath = __dirname + `/cache/deepfry_${tid}_${mid}.png`;

    axios({
        method: 'get',
        url: `https://api-canvass.vercel.app/deepfry?userid=${targetID}`,
        responseType: 'stream',
      })
      .then(response => {
        ...
        writer.on("finish", () => {
          api.sendMessage({
            body: formatFont(`${userName} got deepfried! 😂`),
