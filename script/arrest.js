...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Arrest someone using their profile picture in a meme.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/arrest_${userID}_${targetID}.png`;

  try {
    const arrestURL = `https://api-canvass.vercel.app/arrest?one=${userID}&two=${targetID}`;
    ...
    writer.on("finish", async () => {
      ...
      api.sendMessage({
        body: formatFont(`${name1} just arrested ${name2}! 🚔`),
