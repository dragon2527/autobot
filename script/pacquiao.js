...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Punch someone in a meme using Pacquiao style.",
  aliases: ["pakyaw"],
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/pacquiao_${userID}_${targetID}.png`;

  try {
    const pacquiaoURL = `https://api-canvass.vercel.app/pacquiao?one=${userID}&two=${targetID}`;
    ...
    writer.on("finish", async () => {
      ...
      api.sendMessage({
        body: formatFont(`🥊 ${name1} just threw a punch at ${name2} like Pacquiao!`),
