...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate a Pooh meme using two texts.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (args.length === 0 || !args.join(" ").includes("|")) {
    return api.sendMessage(applyFontMapping("Usage: pooh text1 | text2"), tid, (err, info) => {
  ...
  const outputPath = __dirname + `/cache/pooh_${tid}_${mid}.png`;

  try {
    const apiUrl = `https://api-canvass.vercel.app/pooh?text1=${encodeURIComponent(text1)}&text2=${encodeURIComponent(text2)}`;
