...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate a Nokia meme using someone's profile picture.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/nokia_${userID}_${targetID}.png`;
  ...
  try {
    ...
    let nokiaURL = `https://api.popcat.xyz/nokia?image=${profileURL}`;
    ...
    writer.on('finish', () => {
      api.sendMessage({
        body: formatFont(`${name}'s phone is indestructible like a Nokia! 📱`),
