...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Slap someone using their profile picture.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/slap_${userID}_${targetID}.png`;
  ...
  try {
    ...
    let slapURL = `https://api-canvass.vercel.app/slap?batman=${userID}&superman=${targetID}`;
    ...
    writer.on("finish", () => {
      api.sendMessage({
        body: formatFont(`Oh no!! ${name1} slapped a mosquito named ${name2}! 👋`),
