...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Display user's picture as an ad meme.",
  cooldown: 5,
  hasPrefix: true,
  usage: "ads | ads reply | ads @mention | ads <uid>",
};
...
module.exports.run = async function({ api, event, args }) {
  ...
  api.getUserInfo(targetID, (err, result) => {
    ...
    const outputPath = __dirname + `/cache/ads_${tid}_${mid}.png`;

    axios({
        method: 'get',
        url: `https://api-canvass.vercel.app/adpic?userid=${targetID}`,
        responseType: 'stream',
      })
      .then(response => {
        ...
        writer.on("finish", () => {
          api.sendMessage({
            body: formatFont(`Oh wow there's ad named ${userName}! 😆`),
