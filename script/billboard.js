...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate a Billboard meme using a text.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (args.length === 0) {
    return api.sendMessage(applyFontMapping("❌ | Please provide a text for billboard meme."), tid, (err, info) => {
      setTimeout(() => api.unsendMessage(info.messageID), 3000);
    }, mid);
  }

  const text = args.join(" ").trim();
  const outputPath = __dirname + `/cache/billboard_${tid}_${mid}.png`;
