...
module.exports.config = {
  ...
  role: 0,
  credits: "Kaizenji",
  description: "Generate a Hitler meme using someone's profile picture.",
  cooldown: 5,
  hasPrefix: true,
...
module.exports.run = async function ({ api, event, args }) {
  ...
  const outputPath = __dirname + `/cache/hitler_${userID}_${targetID}.png`;

  try {
    const userInfo = await api.getUserInfo(targetID);
    const name = userInfo[targetID].name;

    let hitlerURL = `https://api-canvass.vercel.app/hitler?userid=${targetID}`;
    ...
    writer.on('finish', () => {
      api.sendMessage({
        body: formatFont(`${name} is worse than Hitler! 💀`),
